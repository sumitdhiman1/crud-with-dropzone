<?php
 function sortorder($fieldname){
        $sorturl = "sort1.php"."?order_by=".$fieldname."&sort=";
        $sorttype = "asc";
   ...
   <thead>
    ...
    <?php
		...
		if(isset($_GET['order_by']) && isset($_GET['sort']) && isset($_GET['pageno == "2"'])){
			 ...
			 $order = "ORDER BY `user`.`email` DESC";
           
        }
		
		 $sql = "SELECT * FROM `user` " .$order." limit $offset, $no_of_records_per_page";
...
?>
               ...
               ?>
                 ...
                 <option  value='?pageno= <?php echo $item; ?>' 
